...
public class ResourceManager
{
    ...
    private void InitializeResources(DefaultResources defaults)
    {
        foreach (ResourceType resourceType in Enum.GetValues(typeof(ResourceType)))
        {
            ...
            var (minStatType, maxStatType, regenStatType, currentValue) =
                defaults.GetValue(resourceType);

            resources[resourceType] =
                new Resource(resourceType, minStatType, maxStatType, regenStatType, currentValue);

            statManager.SubscribeTrue(minStatType, (value) =>
                OnMinStatChanged(resourceType, minStatType, value));
    ...
    private void SetResourceCurrent(ResourceType type, float value)
    {
        var res = resources[type];
        var newValue = value;
        var minValue = statManager.GetValue(res.MinStat);
    ...
    private class Resource
    {
        ...
        public Resource(ResourceType type, StatType minStat, StatType maxStat, StatType regenStat, float currentValue)
        {
            Type = type;
            MinStat = minStat;
            MaxStat = maxStat;
